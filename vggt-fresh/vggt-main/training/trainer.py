...
class Trainer:
    ...
    def train_epoch(self, train_loader):        
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            # Step-based checkpoint (e.g. every 5000 steps)
            save_freq_steps = int(getattr(self.checkpoint_conf, "save_freq_steps", 0) or 0)
            if (
                phase == "train"
                and save_freq_steps > 0
                and self.steps[phase] > 0
                and self.steps[phase] % save_freq_steps == 0
            ):
                self.save_checkpoint(
                    self.epoch,
                    checkpoint_names=["checkpoint", f"checkpoint_step_{self.steps[phase]}"]
                )
